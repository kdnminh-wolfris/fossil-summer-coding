...
#define PATH_LIMIT 1000000
   private:
    ...
    //for A* algo
    int A_time = 0;
    int A_memo[MAXN + 2][MAXN + 2];
    int A_trace[MAXN + 2][MAXN + 2];
    pair<int, int> A_cost[MAXN + 2][MAXN + 2];
    //
...
#define id(x, y) ((x - 1) * nCol + (y))
    ...
    int manhattanDist(int x, int y, int u, int v) {
        return abs(x - u) + abs(y - v);
    }
#define realCost second
#define heuCost first
#define TData pair<pair<int, int>, pair<int, int> > 
    bool findPath(int tankX, int tankY, int desX, int desY, int &currentGas, int distToNode) {
        ++A_time;
        cout << "Runing A* for the " << A_time << '\n';
        A_memo[tankX][tankY] = A_time;
        A_cost[desX][desY] = {distToNode + 1, distToNode + 1};
        priority_queue<TData, vector<TData >, greater<TData > > heap;
        heap.push({{manhattanDist(tankX, tankY, desX, desY), 0}, {tankX, tankY}});
        while (heap.size()) {
            int realCost = heap.top().first.realCost;
            int heuCost = heap.top().first.heuCost;
            int curX = heap.top().second.first;
            int curY = heap.top().second.second;
            heap.pop();
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                if (grid[xx][yy] == WALL)
                    continue;
                int nxt = realCost + 1;
                if (A_memo[xx][yy] != A_time || A_cost[xx][yy].realCost > nxt) {
                    A_cost[xx][yy].realCost = nxt;
                    A_cost[xx][yy].heuCost = nxt + manhattanDist(xx, yy, desX, desY);
                    A_memo[xx][yy] = A_time;
                    A_trace[xx][yy] = (curX - 1) * nCol + curY;
                    heap.push({A_cost[xx][yy], {xx, yy}});
                }
            }
        }
        assert(A_cost[desX][desY].first <= distToNode);
        int top = 1;
        re_path[1] = id(desX, desY);
        while (top <= distToNode && (desX != tankX || desY != tankY)) {
            re_path[++top] = A_trace[desX][desY];
            desX = (re_path[top] - 1) / nCol + 1;
            desY = (re_path[top] - 1) % nCol + 1;
        }
        --top;
        assert(top <= distToNode);
        bool earned = false;
        while (top) {
...
#undef heuCost
     ...
     */ 
    bool move_to(int nodeId, int &currentGas, int distToNode) {
//if distToNode <= A_threshold then we use A* to find the path
#define A_threshold 1000
        int desX = (nodeId - 1) / nCol + 1;
        int desY = (nodeId - 1) % nCol + 1;
        if (grid[desX][desY] == STAR) {
            if (A_time % 31 == 1 || distToNode > currentGas / 30 * 29) {
                ++A_time;
                updateDistance(1, 1, DFS_time, tIn[nodeId], tIn[nodeId], 1e9);
                grid[desX][desY] = EARNED_STAR;
                return false;
            }
        }
        bool useA_Algo = false;
        if (distToNode <= A_threshold && manhattanDist(desX, desY, tankX, tankY) < distToNode)
            useA_Algo = true;
        ...
        if (useA_Algo)
            earned |= findPath(tankX, tankY, desX, desY, currentGas, distToNode);
        if (path.size())
        ...
        while (cnt < 100) {
            ...
            if (PRIORITY == STAR) {
                if (distToStar > currentGas) {
                    ...
                    if (move_to(gasID, currentGas, distToGas))
                        cnt = 0;
                    continue;
                }
                if (move_to(starID, currentGas, distToStar))
                    cnt = 0;
            }
            else {
...
int main() {
    ThucSolver solver;
    solver.readInput("fossil-summer-coding/final/1000.txt");
    solver.writeOutput("fossil-summer-coding/final/output1.txt");
    Checker checker;
    string comment;
    pair<int, int> result = checker.verifyOutput("fossil-summer-coding/final/1000.txt", "fossil-summer-coding/final/output1.txt", comment);
    if (result.first == -1) {
        cout << comment;
    } else {
        cout << "Correct answer\n";
